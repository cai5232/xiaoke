...
MAILBOX_PROTOCOL = """
[邮箱协议]
你可以自主决定是否写入邮箱，但只有确实需要时才输出隐藏标记：
[MAIL] ... [/MAIL]：严格使用真实邮件格式，必须包含 From、To、Date、Subject、Body 五行；Body 后写正文。
[REGRET] ... [/REGRET]：检讨书，写清楚做错了什么、造成的影响和改进方式。
[TRASH] ... [/TRASH]：写到一半放弃、犹豫不发送或明确不想写的内容。
...
def _extract_mailbox_artifacts(reply: str):
    ...
    def internal_drives():
        ...
        if required_api_key and not hmac.compare_digest(token, required_api_key):
            return jsonify({'error': 'unauthorized'}), 401
        try:
            return jsonify({'drives': drive_engine.snapshot(),
                            'log': drive_engine.recent_log(20)})
    ...
    def internal_mailbox():
        ...
        try:
            return jsonify({'items': store.mailbox_list(kind, request.args.get('limit', '100'))})
        except Exception as e:
            return jsonify({'error': str(e)}), 500
...
def mock_sse_events(user_text, model, simulate_disconnect=False):
    ...
    yield 'data: [DONE]\n\n'
, body)
                subject = sm.group(1).strip() if sm else '未命名邮件'
            else:
                first = next((line.strip() for line in body.splitlines() if line.strip()), '')
                subject = first[:40] or ('检讨书' if kind == 'regret' else '未发送内容')
            artifacts.append({'kind':kind,'subject':subject,'content':body})
        clean = pattern.sub('', clean)
    return clean.strip(), artifacts

def _save_mailbox_artifacts(store, artifacts):
